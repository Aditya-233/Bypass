...
class TestBypass(unittest.TestCase):
    ...
    def test_02_http_proxy_port_open(self):
        ...
        try:
            ...
            resp = s.recv(1024)
            self.assertTrue(
                resp.startswith(b"HTTP/1.1 200 Connection Established"),
                f"Unexpected HTTP proxy response: {resp[:50]}",
            )
        finally:
            s.close()
    ...
    def test_03_nyaa_unblocked(self):
        ...
        self.assertEqual(
            res.returncode, 0, f"curl failed with code {res.returncode}: {res.stderr}"
        )
        self.assertIn(
            "200",
            res.stdout.split("\n")[0],
            f"Expected HTTP 200 from nyaa.si, got {res.stdout[:100]}",
        )
    ...
    def test_04_fitgirl_unblocked(self):
        ...
        self.assertIn(
            "200",
            res.stdout.split("\n")[0],
            f"Expected HTTP 200 from fitgirl, got {res.stdout[:100]}",
        )
    ...
    def test_05_bypass_run_wrapper(self):
        ...
        self.assertTrue(
            "200" in res.stdout,
            f"HTTP 200 status not found in response header: {res.stdout}",
        )
    ...
    def test_06_bypass_env(self):
        ...
        bypass_bin = BASE_DIR / "bypass"
        res = subprocess.run(
            [str(bypass_bin), "env"], capture_output=True, text=True, check=False
        )
        ...
        res_un = subprocess.run(
            [str(bypass_bin), "unenv"], capture_output=True, text=True, check=False
        )
